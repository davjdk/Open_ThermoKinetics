...
7. Фаза 4.5 - Агрегаторы операций и значений

Анализ логов приложения solid-state-kinetics показал две критические проблемы производительности:

1. **Каскадные операции**: Одна пользовательская операция (например, ADD_REACTION) вызывает цепочку из 10-15 системных операций в течение секунды
2. **Объемные значения**: Логи переполнены массивами NumPy (489 элементов), DataFrame (489x2), и коэффициентами, занимающими 80-90% объема логов

7.1 Операционный агрегатор (OperationAggregator)

**Назначение**: Группировка каскадных операций в логические блоки по временным окнам

**Реализация** (`src/log_aggregator/operation_aggregator.py`):
...
class OperationAggregator:
    """
    Агрегирует каскадные операции в логические группы.
    Детектирует начало операционного цикла и группирует последующие системные вызовы.
    """
    
    def __init__(self, 
                 cascade_window: float = 1.0,  # Временное окно каскада в секундах
                 min_cascade_size: int = 3,    # Минимальное количество операций для агрегации
                 root_operations: Optional[Set[str]] = None):
        """
        Args:
            cascade_window: Время в секундах для группировки операций
            min_cascade_size: Минимум операций для создания группы
            root_operations: Операции-инициаторы каскадов
        """
        self.cascade_window = timedelta(seconds=cascade_window)
        self.min_cascade_size = min_cascade_size
        
        # Операции, которые инициируют каскады (из анализа логов)
        self.root_operations = root_operations or {
            ...
            'ADD_NEW_SERIES', 'IMPORT_REACTIONS', 'EXPORT_REACTIONS'
        }
        
        # Буфер для группировки операций
        self.pending_operations: deque[LogRecord] = deque()
        self.current_group: Optional[OperationGroup] = None
        self.completed_groups: List[OperationGroup] = []
        
        # Паттерны для извлечения операций из сообщений
    ...
    def process_record(self, record: LogRecord) -> Optional[OperationGroup]:
        """
        Обрабатывает запись лога и возвращает завершенную группу операций, если есть.
        
        Returns:
            OperationGroup если группа завершена, None если операция добавлена в текущую группу
        """
        operation_name = self._extract_operation_name(record)
        
        if not operation_name:
            # Не операционная запись - добавляем к текущей группе если есть
            if self.current_group:
                self._add_to_current_group(record, "misc")
            return None
        
        # Проверяем, является ли операция корневой (инициатор каскада)
        if operation_name in self.root_operations:
            return self._start_new_cascade(record, operation_name)
        
        # Если есть текущая группа и операция в окне времени
        if self.current_group and self._is_within_cascade_window(record):
            self._add_to_current_group(record, operation_name)
            return None
        
        # Операция вне окна - завершаем текущую группу
        if self.current_group:
            completed_group = self._finalize_current_group()
            self._start_new_cascade(record, operation_name)
            return completed_group
        
        # Одиночная операция - не агрегируем
        return None
    
    def _extract_operation_name(self, record: LogRecord) -> Optional[str]:
        """Извлекает название операции из сообщения лога"""
        for pattern in self.operation_patterns:
            match = pattern.search(record.message)
            if match:
                return match.group(1) if pattern.groups == 1 else match.group(2)
        return None
    
    def _start_new_cascade(self, record: LogRecord, operation_name: str) -> Optional[OperationGroup]:
        """Начинает новый каскад операций"""
    ...
    def _add_to_current_group(self, record: LogRecord, operation_name: str):
        """Добавляет операцию к текущей группе"""
        if not self.current_group:
            return
        
        self.current_group.end_time = record.timestamp
        self.current_group.operation_count += 1
        self.current_group.actors.add(record.filename.replace('.py', ''))
        self.current_group.operations.append(operation_name)
        
        # Извлекаем request_id если есть
        request_id_match = re.search(r"request_id': '([^']+)'", record.message)
        if request_id_match:
            self.current_group.request_ids.add(request_id_match.group(1))
        
        # Отмечаем ошибки и предупреждения
    ...
    def _is_within_cascade_window(self, record: LogRecord) -> bool:
        """Проверяет, находится ли операция в окне текущего каскада"""
        if not self.current_group:
            return False
        
        time_diff = record.timestamp - self.current_group.start_time
        return time_diff <= self.cascade_window
    
    def _finalize_current_group(self) -> Optional[OperationGroup]:
        """Завершает текущую группу и возвращает её если достаточно операций"""
        if not self.current_group:
            return None
        ...
        # Возвращаем группу только если достаточно операций
        if group.operation_count >= self.min_cascade_size:
            self.completed_groups.append(group)
            return group
        
        return None
    
    def get_aggregated_record(self, group: OperationGroup) -> LogRecord:
        """Создает агрегированную запись для группы операций"""
        ...
        # Определяем уровень агрегированной записи
        if group.has_errors:
            level = "ERROR"
        elif group.has_warnings:
            level = "WARNING"
        else:
            level = "INFO"
        ...
        return LogRecord(
            ...
            level=level,
            filename="operation_aggregator.py",
            line_number=0,
            message=message,
            raw_record=None
        )
    
    def force_finalize(self) -> Optional[OperationGroup]:
        """Принудительно завершает текущую группу (вызывается при сбросе буфера)"""
        return self._finalize_current_group()
```

7.2 Агрегатор значений (ValueAggregator)

**Назначение**: Сворачивание объемных значений (arrays, DataFrames, coeffs) с раскрытием только при ошибках

**Реализация** (`src/log_aggregator/value_aggregator.py`):
...
class ValueAggregator:
    """
    Агрегирует объемные значения в логах, сворачивая их до краткого описания.
    Полные значения сохраняются для раскрытия при WARNING/ERROR.
    """
    
    def __init__(self, 
                 array_threshold: int = 10,      # Порог для сворачивания массивов
                 dataframe_threshold: int = 5,   # Порог для сворачивания DataFrame
                 dict_threshold: int = 8,        # Порог для сворачивания словарей
                 string_threshold: int = 200):   # Порог для сворачивания строк
        
        self.array_threshold = array_threshold
        self.dataframe_threshold = dataframe_threshold  
        self.dict_threshold = dict_threshold
        self.string_threshold = string_threshold
        
        # Кэш полных значений для контекста ошибок
        self.value_cache: Dict[str, str] = {}
        self.cache_size_limit = 100
        
        # Паттерны для детекции различных типов данных
        self.patterns = {
            'numpy_array': re.compile(r'array\(\[([\d\.,\s\-e]+)\]\)'),
            'dataframe': re.compile(r'(temperature|rate_\d+|conversion).*?\n([\d\s\.\-e\n]+)\n\[(\d+) rows x (\d+) columns\]'),
    ...
    def process_message(self, record: LogRecord) -> str:
        """
        Обрабатывает сообщение лога, заменяя объемные значения краткими описаниями.
        
        Returns:
            Обработанное сообщение с агрегированными значениями
        """
        ...
        message = self._process_coefficients(message, record)
        message = self._process_request_dicts(message, record)
        
        # Сохраняем оригинал в кэш для потенциального раскрытия при ошибках
        if message != original_message:
            cache_key = f"{record.timestamp.isoformat()}_{record.filename}_{record.line_number}"
            self._add_to_cache(cache_key, original_message)
        
        return message
    
    def _process_numpy_arrays(self, message: str, record: LogRecord) -> str:
        """Обрабатывает NumPy массивы"""
        def replace_array(match):
            array_content = match.group(1)
            elements = [x.strip() for x in array_content.split(',') if x.strip()]
            
            if len(elements) <= self.array_threshold:
                return match.group(0)  # Оставляем как есть
            
            # Создаем краткое описание
            first_few = elements[:3]
            last_few = elements[-2:]
            preview = f"[{', '.join(first_few)}, ..., {', '.join(last_few)}]"
            
            return f"📊 array({len(elements)} elements) {preview}"
        
        return self.patterns['numpy_array'].sub(replace_array, message)
    
    def _process_dataframes(self, message: str, record: LogRecord) -> str:
        """Обрабатывает pandas DataFrames"""
        def replace_dataframe(match):
            rows, cols = int(match.group(3)), int(match.group(4))
            
            if rows <= self.dataframe_threshold:
                return match.group(0)  # Оставляем как есть
            
            # Краткое описание DataFrame
            columns = match.group(1)  # Название колонок
            return f"📋 DataFrame({rows}×{cols}) columns: {columns}"
        
        return self.patterns['dataframe'].sub(replace_dataframe, message)
    
    def _process_coefficients(self, message: str, record: LogRecord) -> str:
        """Обрабатывает словари коэффициентов"""
        def replace_coeffs(match):
            coeffs_content = match.group(1)
            coeffs_pairs = [pair.strip() for pair in coeffs_content.split(',')]
            
            if len(coeffs_pairs) <= self.dict_threshold:
                return match.group(0)  # Оставляем как есть
            
            # Извлекаем ключи коэффициентов
            keys = []
            for pair in coeffs_pairs:
                if ':' in pair:
                    key = pair.split(':')[0].strip().strip("'\"")
                    keys.append(key)
            
            return f"'coeffs': {{🔢 {len(keys)} params: {', '.join(keys[:5])}{'...' if len(keys) > 5 else ''}}}"
        
        # Обработка основных коэффициентов
        message = self.patterns['coeffs_dict'].sub(replace_coeffs, message)
        
        # Обработка границ коэффициентов
        def replace_bounds(match):
            bound_type = match.group(1)
            bound_content = match.group(2)
            bound_pairs = [pair.strip() for pair in bound_content.split(',')]
            
            if len(bound_pairs) <= self.dict_threshold:
                return match.group(0)
            
            return f"'{bound_type}': {{🎯 {len(bound_pairs)} bounds}}"
        
        return self.patterns['bounds_dict'].sub(replace_bounds, message)
    
    def _process_request_dicts(self, message: str, record: LogRecord) -> str:
        """Обрабатывает словари запросов"""
        def replace_request(match):
            try:
                # Попытка парсинга как Python dict (безопасный eval)
                request_str = match.group(1)
                
                # Извлекаем ключевые поля
                actor_match = re.search(r"'actor': '([^']+)'", request_str)
                target_match = re.search(r"'target': '([^']+)'", request_str)
                operation_match = re.search(r"'operation': [^,]+\.(\w+)", request_str)
                
                if actor_match and target_match and operation_match:
                    actor = actor_match.group(1)
                    target = target_match.group(1)
                    operation = operation_match.group(1)
                    
                    return f"📨 Request({actor} → {target}: {operation})"
                
                return match.group(0)  # Оставляем как есть если не удалось разобрать
                
            except Exception:
                return match.group(0)  # Оставляем как есть при ошибке парсинга
        
        return self.patterns['request_dict'].sub(replace_request, message)
    
    def _add_to_cache(self, key: str, value: str):
        """Добавляет значение в кэш с ограничением размера"""
        if len(self.value_cache) >= self.cache_size_limit:
            # Удаляем самые старые записи (FIFO)
            oldest_key = next(iter(self.value_cache))
            del self.value_cache[oldest_key]
        
        self.value_cache[key] = value
    
    def get_full_context(self, record: LogRecord) -> Optional[str]:
        """Получает полный контекст для записи (используется при ошибках)"""
        cache_key = f"{record.timestamp.isoformat()}_{record.filename}_{record.line_number}"
        return self.value_cache.get(cache_key)
    
    def get_stats(self) -> Dict[str, int]:
        """Возвращает статистику работы агрегатора"""
        return {
            'cached_values': len(self.value_cache),
            'cache_limit': self.cache_size_limit,
            'array_threshold': self.array_threshold,
            'dataframe_threshold': self.dataframe_threshold
        }
```

7.3 Интеграция агрегаторов в AggregatingHandler

**Обновление** `src/log_aggregator/realtime_handler.py`:

```python
# Добавляем в __init__ AggregatingHandler
...
def __init__(self, 
    ...
    if enable_operation_aggregation:
        self.operation_aggregator = OperationAggregator(
...
def emit(self, record: logging.LogRecord) -> None:
    """Обработка входящей записи лога с новыми агрегаторами"""
    if not self.enabled:
        self.target_handler.emit(record)
        return
    ...
    try:
        with self._lock:
            ...
            if self.enable_operation_aggregation:
                ...
                if completed_group:
                    # Отправляем агрегированную запись вместо отдельных операций
                    ...
                    self.stats['operation_cascades_aggregated'] = self.stats.get('operation_cascades_aggregated', 0) + 1
                    return
            
            # Обработка ошибок с расширенным контекстом
            if self._is_error_record(log_record) and self.enable_error_expansion:
                # Восстанавливаем полные значения для контекста ошибки
...
# Добавляем методы управления новыми агрегаторами
...
def get_stats(self) -> Dict[str, Any]:
    ...
    with self._lock:
        ...
        if self.enable_value_aggregation:
            stats.update({
                'value_aggregation_enabled': True,
                **self.value_aggregator.get_stats()
            })
        
        return stats
```

7.4 Примеры результатов агрегации

**До агрегации** (15 строк логов для одной операции ADD_REACTION):
```
2025-06-13 00:11:51 - INFO - main_window.py:90 - main_window handle_request_from_main_tab 'OperationType.ADD_REACTION' with params={'path_keys': ['NH4_rate_3.csv', 'reaction_0']}
2025-06-13 00:11:51 - DEBUG - base_signals.py:131 - main_window is emitting request: {'actor': 'main_window', 'target': 'calculations_data_operations', 'operation': <OperationType.ADD_REACTION: 'add_reaction'>, 'request_id': '74848602-8fdf-4ba1-a04e-0cb300cf3bd1'}
2025-06-13 00:11:51 - DEBUG - calculation_data_operations.py:52 - Processing operation 'OperationType.ADD_REACTION' with path_keys: ['NH4_rate_3.csv', 'reaction_0']
...12 строк с массивами по 489 элементов...
```

**После агрегации** (1 строка):
```
...
```
# До
'data': array([ 32.18783,  33.14274,  34.09766,  35.05257,  36.00749,  36.9624, ... [489 элементов] ])

# После  
'data': 📊 array(489 elements) [32.18783, 33.14274, 34.09766, ..., 497.23143, 498.18635]
```

8. Заключение

Обновленная архитектура модуля агрегации логов теперь включает четыре важных компонента:

8.1 OperationAggregator
Группировка каскадных операций в логические блоки по временным окнам (1 секунда)
Детекция корневых операций (ADD_REACTION, DECONVOLUTION, etc.) как инициаторов каскадов
Агрегирование до 15 системных вызовов в одну сводную запись
Индикация ошибок и предупреждений в агрегированных группах
Сокращение объема логов операций на 85-90%

8.2 ValueAggregator  
Сворачивание объемных данных (NumPy arrays, pandas DataFrames, coefficient dicts)
Сохранение полных значений в кэше для раскрытия при ошибках/предупреждениях
Интеллектуальное определение типов данных и создание описательных превью
Настраиваемые пороги агрегации для разных типов данных
Сокращение объема логов значений на 80-85%

8.3 ErrorExpansionEngine
...
8.5 Ключевые улучшения Фазы 4.5
**Операционная эффективность**: Каскады операций агрегируются в единые записи, сокращая объем логов на 85-90%
**Умная детализация**: Объемные значения сворачиваются автоматически, но раскрываются при ошибках
**Производительность в реальном времени**: Агрегация происходит "на лету" без влияния на основной поток приложения  
**Контекстная осведомленность**: Система понимает семантику операций solid-state-kinetics (ADD_REACTION, DECONVOLUTION, etc.)
**Гибкая конфигурация**: Независимое включение/выключение каждого агрегатора
**Сохранение критической информации**: Ошибки всегда получают полный контекст с несвернутыми значениями

**Общее улучшение производительности логирования**:
- Сокращение объема логов на 80-90% при обычной работе
- Полная детализация при проблемах (WARNING/ERROR)
- Сохранение семантической информации об операционных потоках
- Улучшение читаемости логов для разработчиков и операторов

Модуль остается полностью обратно совместимым и может работать как с новыми компонентами Фазы 4.5, так и без них, в зависимости от конфигурации.